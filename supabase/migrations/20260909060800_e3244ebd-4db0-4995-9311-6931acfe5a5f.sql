ALTER TABLE public.bonus_ordning
  ADD COLUMN db_bund numeric NULL,
  ADD COLUMN db_top numeric NULL,
  ADD COLUMN maskin_bund numeric NULL,
  ADD COLUMN maskin_top numeric NULL,
  ADD COLUMN total_bund numeric NULL,
  ADD COLUMN total_top numeric NULL,
  ADD COLUMN flatrate numeric NULL,
  ADD CONSTRAINT bonus_ordning_db_graenser CHECK (db_top IS NULL OR db_bund IS NULL OR db_top >= db_bund),
  ADD CONSTRAINT bonus_ordning_maskin_graenser CHECK (maskin_top IS NULL OR maskin_bund IS NULL OR maskin_top >= maskin_bund),
  ADD CONSTRAINT bonus_ordning_total_graenser CHECK (total_top IS NULL OR total_bund IS NULL OR total_top >= total_bund),
  ADD CONSTRAINT bonus_ordning_flatrate_positiv CHECK (flatrate IS NULL OR flatrate >= 0);
...
BEGIN
  ...
  raa AS (
    SELECT o.maaned, o.id AS ordning_id, o.flatrate,
           ...
           db_pr.grundlag * coalesce(o.db_provision_pct, 0) / 100 AS db_raa,
