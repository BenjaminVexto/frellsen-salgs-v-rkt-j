...
-- 1) Snapshot-kolonne for særpris
ALTER TABLE public.quote_lines
  ADD COLUMN IF NOT EXISTS saerpris_kr_snapshot numeric NOT NULL DEFAULT 0;

-- 2) Drop og genopret funktionen med ny signatur (saerpris_kr separat)
...
BEGIN
  ...
  usable AS (
    -- Ingen override. En række er brugbar hvis EN AF de tre værdier er sat.
    -- Særpris og almindelig rabat (pct/kr) kan begge være > 0 på samme række
    -- og stables klient-side via STACK_ORDER.
    SELECT pm.*,
  ...
  ORDER BY u.cust_prio DESC,
           u.prod_prio DESC,
           -- Sær har ikke prioritet over almindelig længere; vi bryder lige på
           -- største samlede "rabat-værdi" (groft, uden listepris).
