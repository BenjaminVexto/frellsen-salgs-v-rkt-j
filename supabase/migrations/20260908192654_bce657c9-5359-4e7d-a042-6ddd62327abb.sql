...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT
      ...
      CASE WHEN _opdel = 'kunde' THEN 1 ELSE count(*)::int END AS antal_kunder
    FROM pr_kunde p
    GROUP BY p.k
  ), kunder AS (
    -- Antal unikke kunder i alt — hash-aggregering over trin 1, ikke rå rækker.
  ...
  SELECT
    ...
    kd.n
  FROM agg a
  CROSS JOIN totaler t
  CROSS JOIN kunder kd
