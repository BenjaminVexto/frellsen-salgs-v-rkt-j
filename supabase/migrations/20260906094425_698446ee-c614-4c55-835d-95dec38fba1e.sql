CREATE TABLE public.produktgruppe_navn (
  afdeling_nr integer NOT NULL REFERENCES public.afdeling(afdeling_nr),
  product_group_1 text NOT NULL,
  navn text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (afdeling_nr, product_group_1)
);

GRANT SELECT ON public.produktgruppe_navn TO authenticated;
GRANT ALL ON public.produktgruppe_navn TO service_role;

ALTER TABLE public.produktgruppe_navn ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Alle kan laese afdelingsnavne"
  ON public.produktgruppe_navn FOR SELECT TO authenticated USING (true);

CREATE TRIGGER produktgruppe_navn_touch
  BEFORE UPDATE ON public.produktgruppe_navn
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

INSERT INTO public.produktgruppe_navn (afdeling_nr, product_group_1, navn) VALUES
  (21, '2',  'Kaffe'),
  (21, '4',  'Te'),
  (21, '6',  'Chokodrik'),
  (21, '8',  'Tedåser & tetilbehør'),
  (21, '10', 'Chokolade & konfekt'),
  (21, '14', 'Slik & konfekt'),
  (21, '20', 'Lakposer'),
  (21, '22', 'Emballage & flødebolleæsker'),
  (11, '14', 'Sukker, sirup & serveringstilbehør')
ON CONFLICT (afdeling_nr, product_group_1) DO UPDATE SET navn = EXCLUDED.navn;

CREATE OR REPLACE FUNCTION public.gruppe_navn(_afdeling_nr integer, _group text)
RETURNS text
LANGUAGE sql
STABLE
...
AS $$
  SELECT coalesce(
    (SELECT n.navn FROM public.produktgruppe_navn n
      WHERE n.afdeling_nr = _afdeling_nr AND n.product_group_1 = _group),
    (SELECT r.navn FROM public.produktgruppe_rolle r
      WHERE r.product_group_1 = _group)
  )
$$;
