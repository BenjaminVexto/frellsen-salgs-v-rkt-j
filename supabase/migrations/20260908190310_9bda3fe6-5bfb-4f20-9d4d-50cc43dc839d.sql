...
BEGIN
  ...
  WITH acc AS (
    SELECT c.id, c.created_in_visma, c.binding_status,
           ...
           EXISTS (
             ...
             WHERE sm.company_id = c.id AND sm.afdeling_nr = 11 AND sm.revenue <> 0
           ) AS har_koebt
    ...
    SELECT a.gkey,
           min(a.created_in_visma) AS foerste,
           bool_or(a.har_koebt) AS har_koebt,
           (array_agg(a.binding_status ORDER BY a.created_in_visma, a.id))[1] AS binding_status
    FROM acc a
    GROUP BY a.gkey
  )
  SELECT date_trunc('month', g.foerste)::date,
  ...
  FROM grp g
  WHERE g.har_koebt
    AND date_trunc('month', g.foerste)::date >= date_trunc('month', _fra)::date
    AND date_trunc('month', g.foerste)::date <= date_trunc('month', _til)::date
    AND date_trunc('month', g.foerste)::date < date_trunc('month', current_date)::date
  GROUP BY 1, 2;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.maalepunkt_nye_kunder_detaljer(_saelger uuid, _fra date, _til date, _kategori text, _maaned date DEFAULT NULL::date)
 RETURNS TABLE(gruppe_key text, gruppe_navn text, gruppe_by text, gruppe_oprettet date, antal_konti integer, company_id uuid, navn text, by text, oprettet date, kundeprisgruppe_2 text, omsaetning numeric, sidste_koeb date)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  ...
  WITH acc AS (
    ...
    WHERE g.har_koebt
      ...
      AND date_trunc('month', g.foerste)::date <= date_trunc('month', _til)::date
      AND date_trunc('month', g.foerste)::date < date_trunc('month', current_date)::date
      AND (_maaned IS NULL OR date_trunc('month', g.foerste)::date = date_trunc('month', _maaned)::date)
      AND (CASE g.binding_status
             WHEN 'frit_salg' THEN 'privat'
             WHEN 'offentlig_aftale' THEN 'offentlig'
             ELSE 'andet' END) = _kategori
  )
  SELECT v.gkey, v.navn, v.by, v.foerste, v.antal_konti,
         a.id, a.name, a.city, a.created_in_visma, a.customer_segment_2,
         round(coalesce(a.omsaetning, 0), 2), a.sidste_koeb
  FROM valgt v
  JOIN acc a ON a.gkey = v.gkey
  ORDER BY v.foerste, v.gkey, a.created_in_visma;
END;
$function$;
