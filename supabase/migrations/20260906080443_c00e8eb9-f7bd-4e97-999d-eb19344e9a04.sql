ALTER TABLE public.companies
  ADD COLUMN IF NOT EXISTS dublet_afvist_at timestamptz NULL;

CREATE OR REPLACE FUNCTION public.dublet_navn_norm(_name text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT btrim(
    regexp_replace(
      regexp_replace(lower(btrim(coalesce(_name, ''))), '^lukket?\s*/\s*', ''),
      '[\s,]*(a/s|aps|i/s|k/s|ivs|as)\.?$', ''
    )
  )
$$;
...
AS $$
  WITH dead AS (
    SELECT c.id, c.name, c.cvr, c.visma_id, c.afdeling_nr, c.created_in_visma,
           c.zip, c.address, c.binding_status, c.dublet_afvist_at,
  ...
  alive AS (
    SELECT c.id, c.name, c.cvr, c.visma_id, c.afdeling_nr, c.created_in_visma,
           c.zip, c.address, c.last_purchase_date, c.last_sales_date,
           c.turnover_12m,
           public.dublet_navn_norm(c.name) AS nn
    FROM public.companies c
    WHERE c.cvr IS NOT NULL AND btrim(c.cvr) <> ''
