...
BEGIN
  ...
  SELECT count(*) INTO v_foerer_relevante
    FROM _norm n
   WHERE n.pct >= 25
     AND n.product_group_1 IN (SELECT product_group_1 FROM _foerer);

  v_mangler_i_alt := coalesce(v_relevante, 0) - coalesce(v_foerer_relevante, 0);
  ...
  RETURN jsonb_build_object(
    ...
    'mangler_i_alt', coalesce(v_mangler_i_alt, 0),
    'foerer', v_foerer,
    'mangler', v_mangler
  );
END
