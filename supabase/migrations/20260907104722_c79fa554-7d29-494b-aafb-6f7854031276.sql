...
BEGIN
  ...
  keys AS (
    SELECT m_cur.period, m_cur.afdeling_nr FROM m_cur
    UNION SELECT p_cur.period, p_cur.afdeling_nr FROM p_cur
    UNION SELECT m_new.period, m_new.afdeling_nr FROM m_new
    UNION SELECT p_new.period, p_new.afdeling_nr FROM p_new
  )
