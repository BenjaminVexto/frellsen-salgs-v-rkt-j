-- Porteføljeaggregeringen ramte statement timeout, fordi RLS på sales_monthly
-- (OR can_access_company, COST 10000) blev evalueret for hver række i vinduet.
-- Funktionerne gøres SECURITY DEFINER og filtrerer selv på brugerens afdelinger
-- via my_afdelinger(), så adgangsreglen er uændret men RLS-overhead forsvinder.
  ...
  WITH valgte AS (
    ...
    SELECT
      ...
      coalesce(array_agg(DISTINCT s.kode) FILTER (
        WHERE s.kode IS NOT NULL AND s.period >= _start_cur AND s.period <= _last_full
      ), '{}'::text[]) AS vare_grupper,
      coalesce(sum(s.rev) FILTER (
        WHERE s.period >= _start_cur AND s.period <= _last_full AND s.kode IN ('2', '4', '6', '10')
...
CREATE OR REPLACE FUNCTION public.portfolio_aggregat_json(_saelger uuid DEFAULT NULL::uuid, _afdeling_nr integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
